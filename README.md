...
Works on Vercel, Railway, Render, or any Node host. Set every variable from `.env.example`, point
`DATABASE_URL` at a real Postgres instance (Neon/Supabase/Vercel Postgres/Railway all work), run
`npx prisma migrate deploy` as part of your build/release step, and configure the cron endpoint above if you
want scheduled posts to publish automatically.
