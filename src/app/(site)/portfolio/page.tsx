...
export default async function PortfolioPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  const items = await prisma.portfolio.findMany({ where, orderBy: { order: "asc" }, include: { coverImage: true } });
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((p) => (
              <div key={p.id} className="rounded-2xl border border-border p-5">
                {p.coverImage && (
                  <div className="relative mb-4 aspect-[16/10] overflow-hidden rounded-xl bg-surface">
                    <Image
